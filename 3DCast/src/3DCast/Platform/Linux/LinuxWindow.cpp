...
void Cast::LinuxWindow::OnUpdate()
{
	glfwPollEvents();
	context->SwapBuffer();
}

inline glm::ivec2 Cast::LinuxWindow::GetPosition() const
{
	...
	{
		warned = true;
		FLOG_CORE_WARN("Retrieving the window position is not supported under wayland. Assuming position (0, 0).");
	}
...
void Cast::LinuxWindow::SetVSync(const bool enabled)
{
	glfwSwapInterval(enabled ? 1 : 0);
	data.VSync = enabled;
}

inline bool Cast::LinuxWindow::IsVSync() const
{
	return data.VSync;
}

inline void Cast::LinuxWindow::SetRawMouseInput(const bool enabled) const
{
...
void Cast::LinuxWindow::SetInputModeDisabled() const
{
...
void Cast::LinuxWindow::Init(const WindowProperties& props)
{
...
void Cast::LinuxWindow::Destroy() const
{
	glfwDestroyWindow(window);
}

void* Cast::LinuxWindow::GetNativeWindow() const
{
	return window;
}

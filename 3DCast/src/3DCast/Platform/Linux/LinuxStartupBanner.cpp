#include "castpch.h"
#include "LinuxStartupBanner.h"

#include "3DCast/Platform/OpenGL/OpenGLContext.h"
#include "3DCast/Core/Log.h"

bool Cast::Core::LinuxStartupBanner::Init(const std::string& imgPath)
{
	...
	shader.reset(API::Core::Shader::Create(std::string(ASSET_DIR) + "shader/sprite/basic_img.vert",
	                                       std::string(ASSET_DIR) + "shader/sprite/basic_img.frag"));
...
void Cast::Core::LinuxStartupBanner::Blit(const unsigned long ms)
{
	const double startTime = glfwGetTime();

	while (!glfwWindowShouldClose(window))
	{
		if ((glfwGetTime() - startTime) * 1000.0 >= (double)ms)
			break;

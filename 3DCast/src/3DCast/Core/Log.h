...
namespace Cast
{
	...
	{
	public:
		[[nodiscard]] const std::vector<std::string>& GetLog() const { return m_LogBuffer; }
	...
	protected:
		...
		{
			/* No flushing required for an in-memory sink */
		}
	...
	public:
		...
		inline static void SetClientLogLevel(const spdlog::level::level_enum level)
		{
		...
		inline static void FlushLogsPeriodically(const unsigned int seconds)
		{
...
#define FLOG_CORE_FATAL(...) ::Cast::Log::GetCoreLogger()->fatal("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
#define FLOG_CORE_ERROR(...) ::Cast::Log::GetCoreLogger()->error("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
#define FLOG_CORE_WARN(...)  ::Cast::Log::GetCoreLogger()->warn("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
#define FLOG_CORE_INFO(...)  ::Cast::Log::GetCoreLogger()->info("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
#define FLOG_CORE_TRACE(...) ::Cast::Log::GetCoreLogger()->trace("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
...
#define FLOG_FATAL(...) ::Cast::Log::GetClientLogger()->fatal("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
#define FLOG_ERROR(...) ::Cast::Log::GetClientLogger()->error("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
#define FLOG_WARN(...)  ::Cast::Log::GetClientLogger()->warn("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
#define FLOG_INFO(...)  ::Cast::Log::GetClientLogger()->info("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
#define FLOG_TRACE(...) ::Cast::Log::GetClientLogger()->trace("[{}:{}] {}", __FILENAME__, __LINE__, fmt::format(__VA_ARGS__))
